...
export default function ConnectionManager({
  ...
  const followUser = async (followId) => {
    ...
    if (response.ok) socket.emit("getUser", user.id);
  };
    ...
    <div className="flex h-min justify-between p-2 bg-l4 dark:bg-d4 rounded-md ">
      ...
      <div className="flex gap-5 flex-none origin-center scale-75 ">
        ...
        >
          ...
          <FollowIcon
            className={`${followLoader ? "hidden" : ""} size-5 fill-d1 dark:fill-l1`}
          />
          <SpinCircle
            className={`${followLoader ? "" : "hidden"} size-5 fill-d1 dark:fill-l1`}
          />
        ...
        >
          ...
          <UnfollowfIcon
            className={`${unfollowLoader ? "hidden" : ""} size-5 fill-d1 dark:fill-l1`}
          />
          <SpinCircle
            className={`${unfollowLoader ? "" : "hidden"} size-5 fill-d1 dark:fill-l1`}
          />
